    ...
    def set_breaking_type(
            ...
            if is_at_market_close:
                ...
                if broke_up and breakout_value > box.low:
                    if proportion_above >= 0.2 or all_in_top_third or compressed_in_top_25:
                        box.box_break_inside = BoxBreakInsideClassification.UP
                    else:
                        box.box_break_inside = None
                elif broke_down and breakout_value < box.high:
                    if proportion_below >= 0.2 or all_in_bottom_third or compressed_in_bottom_25:
...
def get_most_recent_consolidation_box_from_now_to_the_past(
    ...
    for i in range(minutes_passed_since_opening):
        ...
        # Temporary fix for: "Runtime Error: Trying to retrieve an element from a collection using a key that does not exist in that collection throws a KeyError exception.
        # if key_moment can't be found, then don't return a consolidation box
        if key_moment not in all_daily_data:
            return None
