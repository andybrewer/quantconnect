...
class HolyGrailProject(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.target_leverage = getattr(algorithm_parameters, "target_leverage", 2.72)  # e.g., 3×
    ...
    def execute_specific_trade_decider(
            ...
            **kwargs,
            ...
            target_leverage = float(getattr(self, "target_leverage", 2.72))
    ...
    def send_clearstreet_order(self, side: str, quantity: int, action_description: str):
        try:
            data = json.dumps({
                "order_type": "market",
                "side": side,
                "time_in_force": "day",
                "reference_id": str(int(self.time.timestamp())),
                "quantity": str(quantity),
                "symbol": "QQQ"
            })
            headers = {
                "Content-Type": "application/json",
                "x-api-key": "b7d7039509c52964b65a3db2bcf187e8e687db8c3b239858be1c521e5547e6096d36638c86bb581ddccb3112a237e93ae46a0a654a139c7884c0b40e44ab8a86"
            }
            url = f"https://clear-11-2025-201026806113.us-central1.run.app/api/accounts/9100278/orders?env=paper"
            
            self.notify.web(address=url, data=data, headers=headers)
            self.Debug(f"[WEBHOOK] Sent {action_description}: {side} {quantity} QQQ")
        except Exception as e:
            self.Debug(f"[WEBHOOK ERROR] Failed to send {action_description}: {str(e)}")

    def OnOrderEvent(self, order_event: OrderEvent):
        if order_event.Status == OrderStatus.Filled:
            # Get position AFTER the fill
            position_after = self.Portfolio[self.current_ticker].Quantity
            
            # Calculate position BEFORE the fill
            fill_qty = order_event.FillQuantity
            if order_event.Direction == OrderDirection.Sell:
                position_before = position_after + fill_qty
            else:  # Buy
                position_before = position_after - fill_qty
            ...
            if order_event.Direction == OrderDirection.Sell:
                ...
                # Exit Long (positive -> zero/negative)
                if position_before > 0 and position_after <= 0:
                    message = f"RQSI: Exit Long at {order_event.FillPrice}"
                    self.send_clearstreet_order("sell", abs(fill_qty), "Exit Long")
                
                # Partial Exit Long (positive -> still positive)
                elif position_before > 0 and position_after > 0:
                    message = f"RQSI: Partial Exit Long at {order_event.FillPrice}"
                    self.send_clearstreet_order("sell", abs(fill_qty), "Partial Exit Long")
                
                # Enter/Add to Short (zero/positive -> negative)
                else:
                    message = f"RQSI: Enter Short at {order_event.FillPrice}"
                    self.send_clearstreet_order("sell-short", abs(fill_qty), "Enter Short")

                self.notify.sms(phone_number="+18313315114", message=message)
                self.notify.sms(phone_number="+12162245775", message=message)
                ...
                # Exit Short (negative -> zero/positive)
                if position_before < 0 and position_after >= 0:
                    message = f"RQSI: Exit Short at {order_event.FillPrice}"
                    self.send_clearstreet_order("buy", abs(fill_qty), "Exit Short")
                
                # Partial Exit Short (negative -> still negative)
                elif position_before < 0 and position_after < 0:
                    message = f"RQSI: Partial Exit Short at {order_event.FillPrice}"
                    self.send_clearstreet_order("buy", abs(fill_qty), "Partial Exit Short")
                
                # Enter/Add to Long (zero/negative -> positive)
                else:
                    message = f"RQSI: Enter Long at {order_event.FillPrice}"
                    self.send_clearstreet_order("buy", abs(fill_qty), "Enter Long")
